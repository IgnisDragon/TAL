...
class CTRL_Model(object):
    def __init__(self, batch_size=20):
        ...
        self.sentence_embedding_size = 4800 # sentence2vec 得到的维度
        self.visual_feature_dim = 4096 * 3 # (中心 + 上下文) 一共3個，每個由C3D得到是4096维
    ...
    def cross_modal_comb(self, visual_feat, sentence_embed, batch_size):
        ...
        vv_feature = tf.reshape(tf.tile(visual_feat, [batch_size, 1]),
            [batch_size, batch_size, self.semantic_size]) # 因為視頻會有多個，而句子只有一個，所以要做一下維度變化
    ...
    '''
    def visual_semantic_train(self, visual_feature_train, sentence_embed_train):
        
        with tf.variable_scope("CTRL_Model"):
    ...
    def visual_semantic_test(self, visual_feature_test, sentence_embed_test):
        ...
        with tf.variable_scope("CTRL_Model"):
            ...
            sim_score_mat_test = vs_multilayer.vs_multilayer(cross_modal_vec_test, "vs_multilayer_lt", middle_layer_dim=1000)
    ...
    def compute_loss_reg(self, sim_reg_mat, offset_label):
        ...
        sim_score_mat, p_reg_mat, l_reg_mat = tf.split(sim_reg_mat, 3, 2) # 3 * [batch_size, batch_size, 1]
        ...
        batch_para_mat = tf.constant(self.alpha, shape=[self.batch_size, self.batch_size]) # 1.0 / batch_size
    ...
    def construct_model(self):
        ...
        sim_reg_mat = self.visual_semantic_train(self.visual_featmap_ph_train, self.sentence_ph_train)
        sim_reg_mat_test = self.visual_semantic_test(self.visual_featmap_ph_test, self.sentence_ph_test)
        # compute loss
    ...
    def construct_test_model(self):
        # initialize the placeholder
        _, _, _, self.visual_featmap_ph_test, self.sentence_ph_test = self.init_placeholder()
        # build inference network
        sim_reg_mat_test = self.visual_semantic_test(self.visual_featmap_ph_test, self.sentence_ph_test)
