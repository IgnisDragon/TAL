...
def nms_temporal(x1, x2, s, overlap):
    ...
    while len(I) > 0:
        ...
        xx1 = [max(x1[i], x1[j]) for j in I[:-1]] # start
        xx2 = [min(x2[i], x2[j]) for j in I[:-1]] # end
...
def compute_IoU_recall_top_n_forreg(top_n, iou_thresh, sentence_image_mat, sentence_image_reg_mat, sclips, iclips):
    ...
    for k in range(sentence_image_mat.shape[0]): # len(movie_clip_sentences)
        gt = sclips[k] # ex: s13-d21.avi_1_65
        ...
        picks = nms_temporal(starts, ends, sim_v, iou_thresh-0.05) # [0.05, 0.15, 0.25, 0.35, 0.45]
...
def do_eval_slidingclips(sess, vs_eval_op, model, movie_length_info, iter_step, test_result_output):
    ...
    for movie_name in model.test_set.movie_names:
        ...
        for k in range(len(movie_clip_sentences)):
            ...
            for t in range(len(movie_clip_featmaps)):
                ...
                visual_clip_name = movie_clip_featmaps[t][0] # s13-d21.avi_1_65
                start = float(visual_clip_name.split("_")[1]) # 1
                end = float(visual_clip_name.split("_")[2].split("_")[0]) # 65
        ...
        for k in range(len(IoU_thresh)): # IoU(5) * 3 * len(movie_clip_sentences)
            IoU = IoU_thresh[k]
            ...
            print(movie_name + " IoU=" + str(IoU) + ", R@10: " + str(correct_num_10 / len(sclips)) + 
                                "; IoU=" + str(IoU) + ", R@5: " + str(correct_num_5 / len(sclips)) + 
                                "; IoU=" + str(IoU) + ", R@1: " + str(correct_num_1 / len(sclips)))
    ...
    for k in range(len(IoU_thresh)):
        ...
        test_result_output.write("Step " + str(iter_step) + '\n' +
                ": IoU=" + str(IoU_thresh[k]) + ", R@10: " + str(all_correct_num_10[k] / all_retrievd) + '\n' +
                "; IoU=" + str(IoU_thresh[k]) + ", R@5: " + str(all_correct_num_5[k] / all_retrievd) + '\n' +
                "; IoU=" + str(IoU_thresh[k]) + ", R@1: " + str(all_correct_num_1[k] / all_retrievd) + '\n')

def run_training(max_steps=20000, batch_size=56, initial_steps=0):
    #initial_steps = 0
    #max_steps = 20000
    #batch_size = 56
    ...
    with tf.Graph().as_default():
        ...
        sess.run(init)
        load_model_status, global_step = utils.load_ckpt(sess)
